...
void ST7789_Init(void)
{
	...
	/* --- S?A: kéo dài th?i gian reset --- */
	HAL_Delay(20);
	ST7789_RST_Clr();
	HAL_Delay(20);          // gi? RST th?p lâu hon (thay vì 10ms)
	ST7789_RST_Set();
	HAL_Delay(150);         // QUAN TR?NG: ch? t?i thi?u 120ms sau reset (thay vì ch? 20ms)
	/* --- H?T S?A --- */
	...
	HAL_Delay(120);         // S?A: b?t bu?c ch? sau SLPOUT (tru?c dây không có)
